...
const patchOrchestratorProvider = (
...
describe("OrchestratorAgent structured outputs", () => {
  ...
  it("falls back to the default plan and appends a validation risk", async () => {
    ...
    expect(plan.summary).toContain("Coordinate orchestration and worker agents");
    expect(plan.risks.some((risk) => risk.includes("Structured orchestration plan output failed validation"))).toBe(true);
