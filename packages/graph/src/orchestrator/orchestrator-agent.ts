...
const createDefaultPlan = (task: AgentTask): TaskPlan => ({
  summary: `Coordinate orchestration and worker agents to achieve: ${task.goal}`,
  steps: [
    {
      id: "plan",
      title: "Plan the work",
      description: "The orchestrator decomposes the goal into safe, reviewable steps.",
      assignedRole: "leader",
    ...
    },
    {
      ...
      description: "The orchestrator reviews worker output and validation signals.",
  ...
  validationStrategy: [
    ...
    "Require orchestrator review of worker-generated artifacts."
  ]
});
...
export class OrchestratorAgent {
  ...
  public async createPlan(task: AgentTask): Promise<TaskPlan> {
    ...
    return {
      ...
      risks: [
        ...fallbackPlan.risks,
        `Structured orchestration plan output failed validation: ${invocation.errors.join("; ")}`
      ]
    };
  }
  ...
  public async finalize(state: WorkflowState): Promise<AgentResult> {
    ...
    return {
      ...
      agentId: "orchestrator.finalizer",
      role: "leader",
      ...
      artifacts: [
        {
          name: "orchestrator-decision.json",
