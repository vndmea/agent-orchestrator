...
const writeBenchmark = async (rootDir: string): Promise<void> => {
  const benchmarkPath = getWorkerBenchmarkArtifactPath(rootDir, workerId, "coding-v1");
...
describe("buildWorkerAvailabilitySnapshot", () => {
  it("does not report patch-generation as allowed when the persisted profile is internally inconsistent", async () => {
