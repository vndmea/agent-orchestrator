...
- Codex is the only release-validated MCP host integration right now. OpenCode, Claude Code / Claude Desktop, Cursor, and VS Code snippets are untested scaffolding unless `docs/supported-matrix.md` says otherwise.
- Use `cw init` as the default onboarding path. Run it interactively by default, or pass explicit `--worker-id`, `--worker-provider`, `--worker-model`, optional `--worker-base-url` / `--worker-client-command`, `--register-worker`, and `--allow-write` when you need the lower-level scripted setup flow.
- Persist worker, validation, safety, and local client defaults in `config.json`. Provider API keys are managed through `cw auth login`, `cw auth list`, and `cw auth logout` and are stored in the workspace SQLite database.
- Local client adapters are retained as experimental compatibility paths, but the current release-grade worker path is API-model first.
...
- Local client providers and dedicated local adapters are experimental compatibility paths in this release.
- For generic local client experiments, `sparkcode` is the default command. Persist a different compatible local wrapper with `cw init --worker-client-command=<command> --allow-write`.
- For dedicated OpenCode / Claude Code / Codex adapter experiments, keep `provider=opencode`, `provider=claudecode`, or `provider=codex` and override `clientCommand` only when you need a custom executable path.
