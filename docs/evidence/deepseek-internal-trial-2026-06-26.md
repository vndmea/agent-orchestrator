...
- CLI summary output: verified with `ao task report <taskId> --summary`
- MCP summary/full output: verified with `ao_get_task_report`
...
- CLI summary output: verified with `ao task report <taskId> --summary`
- MCP summary output: verified with `ao_get_task_report(detailLevel="summary")`
- MCP full output: verified with `ao_get_task_report(detailLevel="full")`
- MCP artifact read: verified with `ao_read_task_artifact` for `report.md`
...
- AO artifacts stayed under user-scoped AO storage, not the repository checkout.
- No repository-local legacy `.ao` directory was required.
